...
function solutions = stew_forward(L1,L2,L3,L4,L5,L6,xsi,ysi,xmi,ymi,rollmin,rollmax,pitchmin,pitchmax,yawmin,yawmax,pxmin,pxmax,pymin,pymax,pzmin,pzmax,legmin,legmax)
    ...
    baseHeight = -375.4572;
    a1=Ta*[xsi(1);ysi(1);baseHeight;1];              
    a2=Ta*[xsi(2);ysi(2);baseHeight;1];              
    a3=Ta*[xsi(3);ysi(3);baseHeight;1];              
    a4=Ta*[xsi(4);ysi(4);baseHeight;1];              
    a5=Ta*[xsi(5);ysi(5);baseHeight;1];
    a6=Ta*[xsi(6);ysi(6);baseHeight;1]; 
...
%     a6=Ta*[xsi(6);ysi(6);0;1]; 
    ...
    platformHeight = -263.8997;
    b1=Tb*[xmi(1);ymi(1);platformHeight;1];   %b# is the coordinates that will be used for calculation
    b2=Tb*[xmi(2);ymi(2);platformHeight;1];
    b3=Tb*[xmi(3);ymi(3);platformHeight;1];
    b4=Tb*[xmi(4);ymi(4);platformHeight;1];
    b5=Tb*[xmi(5);ymi(5);platformHeight;1];
    b6=Tb*[xmi(6);ymi(6);platformHeight;1];
...
%     b6=Tb*[xmi(6);ymi(6);0;1];
    ...
    %-----------------------------------------------------------------Phi3/4/5
    syms Px Pyy Pz Qx Qy Qz E                %symbolic variables for Px/Py/Pz/Qx/Qy/Qz/E
    ...
    Phi=[simplify((Px^2+Pyy^2+pz1^2-L1^2)*D^2*c1^2)     ;     %Phi3/4/5=PX^2 + PY^2 + PZ^2 - L1^2=0
         simplify((Px^2+Pyy^2+pz2^2-L1^2)*D^2*c2^2)     ;     %other values were added to remove denominators
         simplify((Px^2+Pyy^2+pz3^2-L1^2)*D^2*(c1-c2)^2)];    
    ...
    for i=1:size(c3sol,1)                                           %for each value of c3
        ...
        if (isreal(c1sol)~=0) && (isreal(c2sol)~=0)                 %if c1 and c2 are real
            if double(c1sol) > C_tol                                %and if c1 is bigger than the tolerance value
                ...
                pysol=subs(Pyy,{c1,c2,c3},{c1sol,c2sol,c3sol(i)});   %sub c1/c2/c3 into Py to get py value
                ...
                if double(c2sol) > C_tol                            %if c2 is bigger than the tolerance
                    ...
                    pysol=subs(Pyy,{c1,c2,c3},{0,c2sol,c3sol(i)});   %sub c1/c2/c3 into Py to get py value
                    ...
                    pysol=subs(Pyy,{c1,c2,c3},{c1sol,0,c3sol(i)});   %sub c1/c2/c3 into Py to get py value
